...
int createLabirint(Box* b){
    ...
    int end = 8;//randomEnd(start); // genera un end in posizione casuale
...
char welcome(){
    ...
    printf("Per modalità AI avanzata premere 'c'");
    printf("\n");
    //char mode;
    /*do {
        mode = scanf("%c", &mode);
...
int main(int argc, char *argv[]){
   ...
   // aStarAlgorithm(&board[0], start, 44);
    ...
    if (choose=='b' || choose=='B'){
        ...
        //to be defined
        guide();
        RandomAlgorithm(&board[0], start, score, v);
        printf("%s\n", v);
        free(v);
    }
